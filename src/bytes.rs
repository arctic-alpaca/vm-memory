...
//
// Comment below the bounds as positioning within the bounds breaks rustfmt.
...
pub trait Bytes<A> {
    ...
    fn write_obj<T: Copy + Send + Sync + IntoBytes + Immutable>(
        &self,
        val: T,
        addr: A,
    ) -> Result<(), Self::E> {
    ...
    fn read_obj<T: Copy + Send + Sync + FromZeros + IntoBytes + FromBytes>(
        &self,
        addr: A,
    ) -> Result<T, Self::E> {
